...
__device__ void k_nearest_found_idx_two_stacks_2D(const T_p *tree, const T_t *target, int *found_idx, T_d *dst_k, int K, int TREE_H) {
...
// T_d stands for type of distance, should be float or double to prevent overflow
template<typename T_p, typename T_t, typename T_d>
__device__ void k_nearest_found_idx_two_stacks_compressed_node_2D(const T_p *tree, const T_t *target, int *found_idx, T_d *dst_k, int K, int TREE_H) {
    int node_stack[22 - 1];
    T_d dx2_stack[22 - 1];
